...
static grub_err_t
grub_efi_install_slr_table (void)
{
...
static grub_err_t
grub_efi_locate_mle_offset (grub_ssize_t kernel_start)
{
...
static void *
grub_efi_slaunch_setup_txt_slmem (grub_efi_physical_address_t max_addr,
...
static grub_err_t
grub_efi_slaunch_setup_txt (grub_efi_loaded_image_t *loaded_image)
{
  struct linux_kernel_params *lh = (struct linux_kernel_params *)kernel_addr;
  ...
  slmem = grub_efi_slaunch_setup_txt_slmem ((grub_efi_physical_address_t)addr, &slmem_size);
  if (!slmem)
    {
  ...
  err = grub_efi_locate_mle_offset (start);
  ...
  err = grub_efi_install_slr_table ();
  if (err != GRUB_ERR_NONE)
    goto fail;
...
grub_arch_efi_linux_boot_image (grub_addr_t addr, grub_size_t size, char *args)
{
  ...
  if (grub_slaunch_platform_type () == SLP_INTEL_TXT)
    {
      err = grub_efi_slaunch_setup_txt (loaded_image);
      if (err != GRUB_ERR_NONE)
        {
