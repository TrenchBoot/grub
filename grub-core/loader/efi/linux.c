...
static void *
grub_efi_slaunch_setup_slmem (grub_efi_physical_address_t max_addr,
...
grub_efi_slaunch_setup_txt (grub_efi_loaded_image_t *loaded_image)
{
   ...
   */
  slparams.mle_start = (grub_uint64_t)image_base + start;
  ...
  slmem = grub_efi_slaunch_setup_slmem ((grub_efi_physical_address_t)addr, &slmem_size);
  if (!slmem)
    {
