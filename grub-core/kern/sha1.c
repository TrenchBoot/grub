...
sha1_hash_file( const grub_file_t file, void* result ) {
  ...
  if( sha1_init( &context ) != 0 ) {
      return grub_error (GRUB_ERR_SHA1, N_("sha1_hash_file: hashing failed"));
  }
...
sha1_hash_buffer( const void* buffer, const grub_uint32_t bufferLen, void* result ) {
  ...
  if( sha1_init( &context ) != 0 ) {
      return grub_error (GRUB_ERR_SHA1, N_("sha1_hash_buffer: hashing failed"));
  }
...
sha1_hash_string( const char* string, void* result ) {
	...
	if( sha1_init( &context ) != 0 ) {
        return grub_error (GRUB_ERR_SHA1, N_("sha1_hash_string: hashing failed"));
	}
